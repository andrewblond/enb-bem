...
module.exports = {
    ...
    merge: function (depsToMerge) {
        ...
        this.iterateDeps(depsToMerge, function (key, dep) {
            if (!index[key]) {
                result.push(dep);
                index[key] = true;
            }
    ...
    intersect: function (depsToIntersect) {
        depsToIntersect = [].concat(depsToIntersect);

        var index = {},
            result = [],
            resultDeps = {};

        this.iterateDeps(depsToIntersect, function (key, dep) {
            resultDeps[key] = dep;
        ...
        for (var indexKey in index) if (index[indexKey] > 0) {
            result.push(resultDeps[indexKey]);
        }
    ...
    /**
     * Iterates over deps and calls callback on every item
     *
     * @memberof deps
     * @param {Array} deps
     * @param {Function} callback
     */
    iterateDeps: function (deps, callback) {
        var currentDep, dep, key,
            noop = function () {},
            invoke = typeof callback === 'function' ? callback : noop;

        while (!!(currentDep = deps.shift())) {
